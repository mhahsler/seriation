...
- Linear seriation criterion follows now the original definition of
  Hubert, L. and J. Schultz (1976) and is a merit measure. (reported by
  Lucew)
...
- Fixed typo in parameter name for seriation method ARSA (reported by
  Brian Ripley)
...
- Fixed MDS_angle order for different BLAS implementation giving
  different results for eigen().
...
- permute for dendrograms gained parameter dist and accepts now
  seriation methods.
- Added method “AOE” for correlation matrices.
- registry for seriation methods now contains the name of the seriation
  criterion and a description. seriate_rep now automatically uses the
  criterion from the registry.
...
- pimage and permute now accept order = TRUE to perform the default
  seriation.
- hmap gained parameter col_dist to define the color palette used for
  distance matrices.
- hmap dropped parameter showDend and gained parameter plot_margins
  instead.
...
- tests now also accept reverse orders for testing deterministic
  methods.
...
- The seriation registry now contains help information for the seriation
  method parameters.
- New function seriate_best, seriate_rep, and seriate_improve() to
  easily find a good order for randomized algorithms. Parallel execution
  is supported.
- Seriation method registry has new fields ‘randomized’ to indicate if
  an algorithm is randomized and can be run several times and
  ‘optimizes’ to indicate what criterion is optimized. This information
  is used by seriate_rep.
...
- Embedding-based methods now return the order with an attribute called
  configuration.
...
- New seriation method “SGD” for distances to improve solutions using
...
- New seriation criterion “Rho” calculates the absolute Spearman’s rank
  correlation coefficient.
- list_seriation_methods() and list_criterion_methods() gained parameter
  names_only.

### Changes

- Seriation methods for MDS are now MDS, isoMDS and Sammon_mapping and
  have now individual control parameters.
- orderplot() is now called plot_config() and can also visualize 2D
  configurations.
- HC-based seriation: The control parameter method is now linkage so it
  can be used in seriate() in the …
- Seriation method spectral now also returns the embedding.
- Seriation method simulated annealing is now called “GSA”.
- Simplified generics for pimage and ggpimage. Defaults for pimage.dist
  have changed.
...
- get_order not consistently returns permutation vectors with names (by
  david-barnett).
...
- seriate for arrays (including matrices) now returns a complete
  ser_permutation for all dimensions even if margins are specified. For
  not specified margins, identity permutations are returned.
- added support for tables.
- added new seriation method CA (correspondence analysis) contributed by
  Michael Friendly.
...
- seriate.dist now throws correct error upon encountering NAs (by
  david-barnett)
...
- pimage and dissplot gained parameter diag. pimage for dist by default
  does not show the diagonal now.
...
- Plotting
  - Most plotting functions have now a common interface. This changed
    many parameters.
...
- Other Changes
  ...
  - hclust-based seriations now defaults for linkage to complete instead
    of average.

### New Features

- Plotting
  - Major refactoring of plotting functions to provide a more consistent
    interface.
...
- bertinplot: fix white squares when frame = TRUE (by Dirk
  Seidensticker).
- seriation method “BEA” has now a slight code improvement (suggested by
  RichardKav)
...
- seriation method “BEA” is now not longer masked by “BEA_TSP”. Also the
  FORTRAN calls now work.
- SPIN: making the matrix doubly stochastic now checks all rows/columns
  (reported and fixed by cerebis)
...
- Added criterion Cor_R (ME for the moment ordering algorithm by Deutsch
  and Martin).
...
CRAN release: 2014-12-03

- arsa.f: removed 0 flag in rand() so it compiles under AIX (reported by
  Lei Zhang)
...
- we use now testthat
- fixed bug with intra-cluster ordering using silhouette width (reported
  by Bettina Gruen)
...
CRAN release: 2010-03-13

- service release

## seriation 1.0-1 (8/25/2009)

CRAN release: 2009-08-25
