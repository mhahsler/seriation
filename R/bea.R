...
bea <- function(a, istart = 0, jstart = 0)
{
  ...
  if (istart == 0)
    istart <- floor(runif(1, 1, n))
  if (jstart == 0)
    jstart <- floor(runif(1, 1, m))
  ...
  bea1  <- .Fortran(
    ...
    # permuted array
    ib = as.integer(ib),
    # permuted order of rows
    ifin = as.integer(ifin),
    # for book-keeping
    PACKAGE = "seriation"
  )
  a <- bea1$b
  ...
  bea2  <- .Fortran(
    ...
    a = as.matrix(a),
    # input data
    jstart = as.integer(jstart),
    # 1st col. placement
    b = as.matrix(b),
    # permuted array
    jb = as.integer(jb),
    # permuted order of cols.
    jfin = as.integer(jfin),
    # for book-keeping
    PACKAGE = "seriation"
  )
