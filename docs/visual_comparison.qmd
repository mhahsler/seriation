...
There are too many labels, so we just plot the dendrogram.

```{r}
#| out-width: 100%
#| fig-align: "center"
plot(permute(hclust(dst), order = "OLO", dist = dst))
```

We clearly see some groups of methods that create very similar orderings.
