...
C      TIMEA=DBLE(86400*IDAY+3600*IHR+60*IMIN+ISEC)+DBLE(I100)/100.
      DO I = 1,N
        A(I,I) = 0.0D0
      END DO
C
...
C for bburcg
            IF(A(I,K).GT.A(J,K)+EPS) D(I,J,K)=D(I,J,K)+1
            IF(A(I,K).LT.A(J,K)-EPS) D(I,J,K)=D(I,J,K)-1
C
...
C          WRITE (*,6000) M+1, CHECKS
          ...
          CALL intpr('with following number checks', -1, CHECKS, 1)
...
C    Return R RNG
      ...
      END
C
      SUBROUTINE BOUND2BBURCG(ZBD,N,Q,M,D,S,DD)
      IMPLICIT INTEGER(A-Z)
...
C 98   FORMAT(9I7)
      ...
      END
C
      SUBROUTINE EVALBBURCG(ZBD,Q,N,D)
      IMPLICIT INTEGER(A-Z)
      INTEGER Q(N),D(N,N,N)
      ZBD=0
      DO 85 I = 1,N
