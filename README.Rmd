...
Seriation arranges a set of objects into a linear order given available data with the goal to reveal structural information.  
...
matrices, correlation matrices, and dendrograms (see below for a full list). 
To reveal structural information, the package provides several visualizations (grid and ggplot2) 
including permuted image plots, reordered heatmaps, Bertin plots, and clustering visualizations like dissimilarity plots, and
visual assessment of cluster tendency plots (VAT and iVAT).

Here are some quick guides for

* [reordered heatmaps.](https://mhahsler.github.io/seriation/heatmaps.html)
* [reordered correlation matrices](https://taiyun.github.io/corrplot/) (using seriation in package corrplot).
...
```{r, echo = FALSE, results='asis'}
rev_deps <- tools::package_dependencies("seriation", recursive = FALSE, reverse = TRUE, 
                                                                    which = c("Depends", "Imports", "Suggests"))[[1L]]

format_cran_links <- function(pkgs) {
  paste0("[", pkgs,"]","(https://CRAN.R-project.org/package=", pkgs, ")")
}
                            
cat(paste(format_cran_links(rev_deps), collapse = ", "))
```
...
between Supreme Court Judges during the period from 1995 to 2002. The goal
...
Compare the available seriation criteria. Seriation clearly improved all (Note: some 
